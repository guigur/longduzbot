...
class Eco(commands.Cog):
	...
	async def wad(self, ctx, arg = None):
		...
		money = self.database.getDBMoneyVerif(ctx.author, ctx.guild)
		userS = userStruct(ctx.author.name, ctx.author.discriminator, ggr_utilities.userIcon(ctx.author), money[5])
	...
	async def topwad(self, ctx):
		...
		if (users != None and len(users) == 3):
			...
			try:
				...
				ggr_utilities.logger("Error generating the topwad card", self, None, ggr_utilities.LogType.ERROR)
				await ctx.send("Erreur lors dee la creation la card")
		else:
			ggr_utilities.logger("Not enought members in the money database", self, None, ggr_utilities.LogType.INFO)
